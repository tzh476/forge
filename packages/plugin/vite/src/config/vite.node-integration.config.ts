...
export function pluginNodeIntegration(): Plugin {
  return {
    ...
    resolveId(source, importer) {
      if (!nodeIntegrationModules.has(source)) return;
      // The shim itself does `require("electron")`. Without this guard that
      // request resolves back to the shim, so the generated module initialises
      // itself: `moduleValue = (init_x(), ...)`. `__esmMin`'s `fn = 0` guard
      // swallows the re-entrant call rather than overflowing the stack, so
      // every export silently becomes `undefined`. Leaving it unhandled is
      // worse -- the bundler would resolve `electron` to the npm package, which
      // outside an Electron process is the installer stub. Marking it external
      // keeps the require in the output for the Electron runtime to satisfy.
