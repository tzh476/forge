...
`,
  ...
  it('preserves Node and Electron imports in production builds', async () => {
    ...
    // Assert the property that matters -- the specifier still reaches a
    // runtime `require` -- rather than one bundler's spelling of it. Rollup
    // emits a direct `require("electron")`; Rolldown wraps it in an interop
    // helper whose call reads `require.apply(this, arguments)`, so a regex
    // anchored on `require(` alone reports a false negative on a correct
    // bundle. Matching the specifier is what both have in common.
    for (const specifier of ['electron', 'node:fs', 'node:path', 'node:os']) {
      const reachesRuntime = new RegExp(
        `require[\\s\\S]{0,400}?${specifier.replace(':', '\\:')}`,
      ).test(output);
      expect(
        reachesRuntime,
        `expected "${specifier}" to survive as a runtime require; got:\n${output.slice(0, 2000)}`,
      ).toBe(true);
  ...
  it('keeps user dependency and bundler settings', async () => {
